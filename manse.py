    ...
    try:
        if is_today:
            ...
            _SW_D = {
                ...
                "傷官":"말조심, 윗사람과의 마찰을 피하게. 창의성은 좋으나 충돌 주의니라.",
            ...
            out.append(f"올해({current_year}년) {sw.get('세운','')} 세운의 기운 안에서 오늘 하루가 펼쳐지느니라.\n")
            out.append(f"현재 흐르는 십성: **{sw_ss}** | 길흉: **{sw_gh}**\n")
            out.append(_SW_D.get(sw_ss, f"{sw_ss} 기운이 오늘 하루에도 그대로 흐르느니라.") + "\n")
            sw_n = get_yearly_luck(pils, current_year + 1)
            out.append(f"\n내년 {current_year+1}년은 {sw_n.get('세운','')} [{sw_n.get('십성_천간','')}] 기운이 다가오고 있으니 미리 내다보게.\n")
            ...
            _ILJJ_LOVE = {
                "子":"지혜롭고 다정한 배우자 기운. 지적 교감과 대화가 관계의 핵심이니라.",
                "丑":"성실하고 현실적인 배우자 기운. 안정과 책임감을 중시하느니라.",
                "寅":"진취적이고 리더십 강한 배우자 기운. 독립심이 강하니 존중이 필수니라.",
                "卯":"섬세하고 예술적 감수성의 배우자 기운. 감성 교류가 사랑의 언어니라.",
                "辰":"능력 있고 카리스마 강한 배우자 기운. 단 고집이 센 면이 있느니라.",
                "巳":"두뇌 명석하고 표현력 강한 배우자 기운. 열정적 사랑을 추구하느니라.",
                "午":"열정적이고 활발한 배우자 기운. 외향적이고 솔직한 애정 표현이 특징이니라.",
                "未":"온화하고 예술적 감수성의 배우자 기운. 배려심이 깊고 감성이 풍부하느니라.",
                "申":"냉철하고 실리적인 배우자 기운. 현실 판단이 빠르고 독립적이니라.",
                "酉":"세련되고 완벽주의적 배우자 기운. 까다로울 수 있으나 깊은 헌신이 있느니라.",
                "戌":"의리 있고 뜨거운 열정의 배우자 기운. 한번 마음을 열면 평생 지키느니라.",
                "亥":"지혜롭고 깊은 내면의 배우자 기운. 자유 기질이 있어 구속을 싫어하느니라.",
            }
            ...
            _GKS = {
                "정관격":"규칙과 질서를 중시하며 조직에서 빛을 발하는 격국이니라. 공직·관리직에서 크게 성취하는 팔자이니라.",
                "편관격":"강인한 의지와 도전 정신이 핵심이니라. 군경·의료·스포츠에서 진가를 발휘하느니라.",
                "정재격":"성실함으로 재물을 쌓는 격국이니라. 금융·부동산에서 두각을 나타내느니라.",
                "편재격":"사업가 기질이 넘치는 격국이니라. 투자·영업에서 두각을 나타내느니라.",
                "식신격":"복록이 넘치는 격국이니라. 창작·예술·교육에서 자연스럽게 빛을 발하느니라.",
                "상관격":"재기와 창의성이 폭발하는 격국이니라. IT·예술·방송에서 독보적 존재가 되느니라.",
                "편인격":"학문과 연구에 뛰어난 격국이니라. 철학·의학·IT 연구직에서 전문성을 쌓아가느니라.",
                "정인격":"학문과 자격의 격국이니라. 교육·학술·전문직에서 평생 성장하느니라.",
            }
            out.append(f"**{name}의 사주 종합 분석**\n일간 {ilgan} | 격국 **{gkn}** | {sn}(점수 {sc}/100)\n")
            ...
            sw = get_yearly_luck(pils, current_year)
            out.append(f"\n올해({current_year}년)는 {sw.get('세운','')} [{sw.get('십성_천간','')}] {sw.get('길흉','')} 기운이니라.\n")
...
def build_past_events(pils, birth_year, gender):
    ...
    정확도 향상 포인트:
    · 천간충(甲-庚, 乙-辛, 丙-壬, 丁-癸) + 지지충 동시 발생 → 최고강도
    · 육합(六合) 감지로 긍정 이벤트 추가
    · 충 종류별 도메인 자동 매핑 (子午→건강, 丑未→재물손실, 寅申→사고 등)
...
[충·합별 사건 해석 규칙]
• 천간충 (甲↔庚, 乙↔辛, 丙↔壬, 丁↔癸) → 의지 충돌, 직장·사업 결정적 변동
• 지지충 (子↔午, 丑↔未, 寅↔申, 卯↔酉, 辰↔戌, 巳↔亥) → 생활 기반 흔들림, 이사·이직·사고
...
def _nar_ch1_ilgan(ctx):
    """1~2장: 일간 캐릭터 + 신강신약"""
    ilgan        = ctx['ilgan']
    ilgan_kr     = ctx['ilgan_kr']
    iljj         = ctx['iljj']
    iljj_kr      = ctx['iljj_kr']
    display_name = ctx['display_name']
    birth_year   = ctx['birth_year']
    sn           = ctx['sn']
    strength_info= ctx['strength_info']
    char         = ctx['char']
    sn_narr      = ctx['sn_narr']
    return '\n'.join([
f"",
...
def _nar_ch8_flow(ctx):
    ...
    cur_dw       = ctx['cur_dw']
    cur_dw_ss    = ctx['cur_dw_ss']
    sw_now       = ctx['sw_now']
    sw_next      = ctx['sw_next']
    yongshin_ohs = ctx['yongshin_ohs']
    ilgan_oh     = ctx['ilgan_oh']
    return '\n'.join([
f"",
f"",
        ...
        def _local_saju_response(query):
            ...
            try:
                if is_year:
                    ...
                    # 4️⃣ 필살의 비방
                    _BIYB = {
                        "木":"새벽 인시(寅時 03~05시)에 동쪽 창문을 열고 파란 옷 입은 채 깊게 호흡하게. 목기(木氣)가 충전되느니라.",
                        "火":"정오 오시(午時 11~13시)에 남쪽 방향으로 붉은 소품을 놓게. 화기(火氣)의 밝음이 운을 열어주느니라.",
                        "土":"환절기에 황색 음식(꿀·고구마)을 먹으며 중심을 잡게. 토기(土氣)가 안정을 가져오느니라.",
                        "金":"서쪽 방향 책상 위에 금속 소품을 놓고 결단력을 다지게. 금기(金氣)가 길을 열어주느니라.",
                        "水":"해시(亥時 21~23시)에 북쪽 방향으로 검은 물컵을 두게. 수기(水氣)가 지혜를 불러오느니라.",
                    }
                    out.append(f"\n**4️⃣ [필살의 비방]**\n")
    ...
    if st.button("📥 PDF 생성 및 다운로드", use_container_width=True, key="pdf_gen_btn"):
        try:
            ...
            if include_yongshin:
                ...
                # 기신 경고
                _GISIN_WARN = {
                    "木": "기신(忌神)이 목(木) 기운이니 목 관련 해(寅·卯년)에는 무리한 확장을 삼가게.",
                    "火": "기신이 화(火) 기운이니 화 관련 해(巳·午년)에는 심장·혈압 건강을 챙기고 충동적 결정을 자제하게.",
                    "土": "기신이 토(土) 기운이니 토 관련 해(辰·戌·丑·未년)에는 부동산 거래와 이사를 신중히 하게.",
                    "金": "기신이 금(金) 기운이니 금 관련 해(申·酉년)에는 수술·부상을 조심하고 투자를 자제하게.",
                    "水": "기신이 수(水) 기운이니 수 관련 해(亥·子년)에는 신장·방광 건강을 챙기고 유동성 투자를 줄이게.",
                }
